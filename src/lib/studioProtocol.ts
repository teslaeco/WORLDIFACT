...
export type StudioInput = { worldId: 'enchanted-ai-shop' | 'ai-game-lab'; prompt: string; purpose: 'game' | 'figurine' | 'terrain' | 'object'; textureMaxSize: TextureLimit; photos: StudioPhoto[] }
...
export type StudioStatus = { ready: boolean; publicPilot: boolean; reason: string; oracle: string; photoReady: boolean; promptMaxLength: number; allowance: { used: number; limit: number; remaining: number; enabled: boolean; expiresAt: string | null } | null }
