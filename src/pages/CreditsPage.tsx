...
function CreditsContent({ user, loading }: { user: AccountUser | null; loading: boolean }) {
  ...
  return <main className="credits-page">
    ...
    <section className="credits-plans" aria-label="Generation plans">
      <article><span className="credits-plan-tag">EXPLORE</span><h2>Free</h2><div className="credits-price"><strong>$0</strong><span>Get to know your creative tools.</span></div><ul><li><b>2 FAST generations</b> in every rolling 24 hours</li><li>Download your FAST 3D drafts</li><li><b>1 SLOW generation</b> per UTC calendar day</li><li>Download completed owned FAST or SLOW model files</li></ul>{balance && <div className="credits-remaining">Available now: {balance.free.fastRemaining} FAST · {balance.free.slowRemaining} SLOW</div>}<Link className="credits-action secondary" to={user ? '/shop' : signInHref}>{user ? 'Create a model' : 'Create a free account'} ↗</Link></article>
      <article className="credits-featured">
        ...
        <fieldset className="credits-purchase-choice" disabled={!!busy}>
          ...
          <label className={recurring ? 'selected' : ''}><input type="radio" name="purchase-kind" value="subscription" checked={recurring} onChange={() => setPurchaseKind('subscription')} /><span><strong>Monthly membership</strong><small>Credits every paid month + active membership benefits</small></span></label>
        ...
        <ul><li>Use credits for FAST or SLOW generation</li><li>1,500 credits {recurring ? 'after every confirmed monthly payment' : 'after a confirmed one-time payment'}</li><li>{recurring ? 'Active membership plus a new 1,500-credit grant each paid month' : 'Add more credits whenever you need them'}</li></ul>
        <div className="credits-access-note" id="credits-access-note">
          <strong>{recurring ? 'Monthly membership adds recurring credits and account benefits.' : 'A one-time top-up adds credits only.'}</strong>
          <p>{recurring ? 'You pay $29.99 USD each month until you cancel. Every paid month adds 1,500 credits and gives you active membership.' : 'This payment does not activate or extend membership. Completed owned model downloads are already available; this payment only changes your credit balance.'}</p>
          {!recurring && !member && <label><input type="checkbox" checked={understandsPack} disabled={!user || !!busy} onChange={event => setUnderstandsPack(event.target.checked)} /><span>I understand that a one-time top-up adds credits but does not activate monthly membership.</span></label>}
        </div>
    ...
    <p className="credits-footnote">FAST creates an Astra-guided procedural draft. SLOW uses the detailed model pipeline; texture availability depends on the generated result. Completed owned SLOW model files remain downloadable when the worker has produced that format. A GAME model still needs separate validation for physical manufacturing.</p>
