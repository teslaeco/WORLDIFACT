...
const REASONS: Record<string, string> = {
  DISABLED_OR_EXPIRED: 'The server generation window is disabled or expired. Your description and images remain editable. No request has been charged.',
  RECEIPT_SECRET_MISSING: 'The server receipt service needs configuration. No request has been charged.',
  ALLOWANCE_UNAVAILABLE: 'The server could not read the remaining allowance. It will not guess or start a paid job.',
  ALLOWANCE_EXHAUSTED: 'The approved cumulative generation allowance is exhausted. Existing models can still be recovered; a new allowance needs owner approval.',
  ORACLE_NOT_READY: 'The existing Oracle/Blender worker is not confirming readiness. No model request has been started.',
  OWNER_ACCESS_REQUIRED: 'This window requires the existing owner access code. This is not your OpenAI API key or a ChatGPT login.',
...
/** Same-origin UI, using the existing Oracle worker. No cross-site login iframe. */
export default function ShopPage() {
  ...
  const [preview, setPreview] = useState<{ blob: Blob; url: string; label: string; warning: string } | null>(null)
  ...
  const showBlob = async (blob: Blob, label: string, token: number) => {
    let warning = ''
    ...
    setPreview({ blob, url, label, warning })
  }
  const refresh = async () => {
    if (checking) return
    setChecking(true)
  ...
  useEffect(() => {
    ...
    try {
      coordinator.current = new StudioCoordinator(window.localStorage)
      const restored = coordinator.current.restore()
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Recovery storage is unavailable. Generation is paused.') }
    setChecking(true)
    checkStudio().then(value => { if (mounted.current) setStatus(value) })
      .catch(() => { if (mounted.current) setError('The server status is unavailable. You can still prepare your description and images.') })
      .finally(() => { if (mounted.current) setChecking(false) })
    listStudioModels().then(value => { if (mounted.current) setArchive(value) }).catch(() => {})
    return () => { mounted.current = false; epoch.current++; if (objectUrl.current) URL.revokeObjectURL(objectUrl.current) }
  }, [])
  ...
  const loadResult = async (selected: SavedStudioJob) => {
    ...
    try {
      const blob = await coordinator.current.artifact('model', selected)
      if (!mounted.current || token !== epoch.current) return
      await showBlob(blob, selected.prompt, token)
      try {
    ...
    finally { if (mounted.current && token === epoch.current) setArtifactBusy(false) }
  }
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (stopped) return
        failures++
        setError(e instanceof Error ? e.message : 'Status temporarily unavailable. Recover the same job.')
        if (failures >= 4) return
      }
      if (!stopped) timer = setTimeout(poll, STUDIO_POLL_MS * Math.min(failures + 1, 3))
    }
    // Submission has its own response. Do not race it with an immediate GET.
    timer = setTimeout(poll, 1500)
    return () => { stopped = true; if (timer) clearTimeout(timer) }
    // Changes to job status must not restart the request loop.
  ...
  const generate = async (event: React.FormEvent) => {
    ...
    if (busy || photoBusy || saved || !status?.ready || !coordinator.current) return
    ...
    try {
      ...
      const value = await coordinator.current.start(input, record => { if (mounted.current) { setSaved(record); setJob({ id: record.receipt.id, state: 'pending', detail: JOB_DETAILS.pending }) } }, owner)
      if (mounted.current) setJob(previous => previous?.id === value.id && terminal(previous.state) ? previous : value)
  ...
  const addPhotos = async (files: FileList | null) => {
    if (!files || saved || photoBusy) return
    ...
    try {
      ...
      const views = ['front', 'side', 'back', 'detail'] as const
  ...
  const newModel = () => {
    if (busy || !window.confirm('Start a separate model? This leaves the selected job; it does not cancel its server work. Keep any downloaded model.')) return
    try { coordinator.current?.clearSelection(); setSaved(null); setJob(null); clearPreview(); setArtifactBusy(false); setError(''); setNotice(''); setSeconds(0) }
    catch (e) { setError(e instanceof Error ? e.message : 'Could not release the current selection.') }
  }
  const exportFile = async (format: 'pbr' | 'fbx' | 'blend') => {
    if (artifactBusy || !saved || job?.state !== 'succeeded' || !coordinator.current) return
    setArtifactBusy(true)
    ...
    catch (e) { setError(e instanceof Error ? e.message : 'This export is not available on the connected worker.') }
    finally { if (mounted.current) setArtifactBusy(false) }
  }
  const canGenerate = !!status?.ready && (!photos.length || status.photoReady) && !busy && !photoBusy && !saved && prompt.trim().length >= 3

  return <main className="portal-page native-shop">
    <header className="native-shop-nav">
      <Link to="/" className="native-shop-back">← Back to WORLDIFACT</Link>
      <strong>AI Shop · 3D Studio</strong>
      <nav aria-label="World portals">{PORTALS.map(portal => <Link key={portal.id} to={portal.route}>{portal.shortTitle}</Link>)}</nav>
    </header>
    <section className="native-shop-workspace" aria-label="Create and review a 3D model">
      <div className="native-shop-preview">
        <span className="eyebrow">{saved || preview ? 'YOUR MODEL · GENERATED / UNREVIEWED' : 'EXISTING FORGE CHARACTER · EXAMPLE ONLY'}</span>
        {preview ? <>
          ...
          <button type="button" onClick={() => download(preview.blob, `WORLDIFACT-${saved?.receipt.id || 'archived'}.glb`)}>Download GLB + embedded materials · {(preview.blob.size / 1048576).toFixed(1)} MB</button>
        </> : saved ? <div className="native-shop-progress" role="status">
          <h2>{artifactBusy ? 'Loading your generated model…' : job?.state === 'failed' ? 'This job needs attention' : 'Your model job'}</h2>
          <p>{job?.detail || JOB_DETAILS.pending}</p><p>Elapsed: {Math.floor(seconds / 60)}m {seconds % 60}s · no invented progress percentage</p>
          <p>Job: <code>{saved.receipt.id}</code></p>
        </div> : <>
        ...
        {saved && <div className="native-shop-actions">
          <button type="button" disabled={busy || artifactBusy} onClick={() => job?.state === 'succeeded' ? void loadResult(saved) : setRetry(v => v + 1)}>Recover this job / reload result</button>
          {job?.state === 'succeeded' && <><button disabled={artifactBusy} onClick={() => void exportFile('pbr')}>Download available PBR textures</button><button disabled={artifactBusy} onClick={() => void exportFile('fbx')}>FBX</button><button disabled={artifactBusy} onClick={() => void exportFile('blend')}>Blender</button></>}
        </div>}
      </div>
      <div className="native-shop-form">
        ...
        <form onSubmit={generate}>
          <label htmlFor="studio-prompt">Describe your model</label>
          <textarea id="studio-prompt" value={prompt} maxLength={4000} rows={6} disabled={busy || !!saved} onChange={e => setPrompt(e.target.value)} placeholder="For example: a realistic skull study with ivory bone materials, a stable base and clearly defined teeth." required />
          <label htmlFor="studio-purpose">Purpose</label>
          <select id="studio-purpose" value={purpose} disabled={busy || !!saved} onChange={e => setPurpose(e.target.value as StudioInput['purpose'])}><option value="figurine">Figurine or chess piece</option><option value="game">Game asset</option><option value="terrain">Terrain or relief</option><option value="object">Custom object</option></select>
          <label htmlFor="studio-texture">Requested texture-size ceiling</label>
          <select id="studio-texture" value={textureLimit} disabled={busy || !!saved || !!photos.length || photoBusy} onChange={e => setTextureLimit(Number(e.target.value) as TextureLimit)}><option value={2048}>Up to 2K</option><option value={4096}>Up to 4K</option><option value={8192}>Up to 8K</option></select>
          <small>This is a requested ceiling, not guaranteed detail. References are never upscaled. Actual texture quality depends on the worker and source images.</small>
          <label className="native-shop-upload" htmlFor="studio-photos">{photoBusy ? 'Preparing reference images…' : 'Add reference images · JPG / PNG / WebP'}</label>
          <input id="studio-photos" type="file" className="native-shop-file" multiple accept="image/jpeg,image/png,image/webp" disabled={busy || !!saved || photoBusy} onChange={e => { void addPhotos(e.target.files); e.target.value = '' }} />
          <small>Up to 4 views of the same object. Maximum 12 MB per original and 6 MB combined after preparation.</small>
          ...
          <button className="native-shop-generate" type="submit" disabled={!canGenerate}>{busy ? 'Submitting this model…' : 'Generate 3D model + materials'}</button>
          <small>Starts one model job using the existing API account and approved allowance. It does not download a brief. Downloads and catalog publication are separate actions.</small>
        </form>
        ...
        {saved && <button type="button" disabled={busy || artifactBusy} onClick={newModel}>Start a separate model…</button>}
    ...
    <section aria-labelledby="studio-archive-title"><h2 id="studio-archive-title">Your models · device archive</h2><p>Completed originals are saved on this device, not automatically published to a store. Clearing browser storage removes this archive; keep explicit file backups.</p><label>Find a saved model<input type="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search descriptions" /></label><div className="native-shop-archive">{archive.filter(item => item.prompt.toLowerCase().includes(search.toLowerCase())).map(item => <article key={item.id}><strong>{item.prompt}</strong><small>{(item.byteLength / 1048576).toFixed(1)} MB · UNREVIEWED</small><button type="button" disabled={busy || (!!saved && !terminal(job?.state)) || artifactBusy} onClick={() => { clearPreview(); const token = epoch.current; readStudioModel(item.id).then(blob => showBlob(blob, item.prompt, token)).catch(e => setError(e.message)) }}>Open saved model</button></article>)}</div>{archive.length === 0 && <p>No models saved on this device yet. Existing private Froge archives have not been copied or deleted.</p>}</section>
