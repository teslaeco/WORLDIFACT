...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    // A job status update must not restart its own request loop.
  ...
  const newModel = () => {
    if (operations.current.submit || operations.current.artifact || !window.confirm('Start a separate model? This leaves the selected job; it does not cancel its server work. Keep any downloaded model.')) return
    try { coordinator.current?.clearSelection(); setSaved(null); setJob(null); clearPreview(); setError(''); setNotice(''); setSeconds(0) }
