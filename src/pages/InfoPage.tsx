...
export default function InfoPage({ kind }: { kind: "privacy" | "terms" }) {
  return (
    <main className="info-page">
      ...
      <p>Updated 22 September 2026. WORLDIFAKT is an early playable prototype.</p>
      {kind === "privacy" ? <>
        ...
        <p>LIVE preview generation is subject to operational rate limits, input limits, provider availability and abuse protection. Free accounts receive two FAST generations in a rolling 24-hour period and one SLOW generation per UTC day. Completed owned FAST and SLOW model files may be downloaded when the worker has produced the requested format. Credit-funded generation costs 50 credits. Membership costs $29.99 USD per month and renews monthly until cancelled. Each paid month grants 1,500 credits (30 generations); the same $29.99 USD price also applies to a one-time top-up of 1,500 credits. Choose monthly membership or a one-time top-up in the single paid offer. A one-time top-up does not require, activate or extend a subscription. Downloads do not require a separate membership entitlement. One-time top-ups have no automatic renewal. PayPal supports one-time top-ups only; monthly membership uses Stripe checkout with card payment and Google Pay where eligible. Monthly membership checkout remains unavailable until its payment configuration is complete. The amount and any renewal period are shown before checkout. No payment is inferred from a return URL; only server-verified provider payment data changes the balance. Refunded or disputed purchases can have their associated credit grant removed and require account review; repeated events never add or remove the same grant twice. Confirmed generation failures restore the consumed allowance or credits. Uncertain jobs remain reserved pending reconciliation to prevent double generation or billing. Completed owned GLB files can be used as previews and downloads; optional PBR/FBX/BLEND exports depend on the saved worker job and may require no-AI post-processing. Keep exported copies of important work. DEMO remains available when live generation is unavailable.</p>
