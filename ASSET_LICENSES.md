...
- WORLDIFACT build preparation downloads only that exact immutable upstream GLB, verifies its 10,343,368-byte length, GLB header and SHA-256 `4b7e83d07723be958e7325f1cd7afc509ebc72d61a6357925c824ac716052adf`, and publishes the verified bytes as the same-origin runtime asset `/world-assets/forge/rapper-v10.glb`. Production release smoke compares the published file with the exact build bytes.
- Runtime validates the expected GLB container/hash again, normalizes only scene scale/ground position, and clones the same verified generic-adult source for bounded GAME NPC instances. Geometry is not called manufacturing-ready.
- The pinned ForgeMPC2 revision contains one committed character GLB, not a library of multiple distinct committed character binaries. Different WORLDIFACT workers therefore use verified clones with different GAME tasks/props rather than falsely claiming unavailable source characters.
- If the same-origin asset cannot be fetched on a client, WORLDIFACT keeps clearly procedural fallback workers so Queen/player/portal startup is not blocked. The fallback is original GAME geometry and is not represented as the Forge asset.
