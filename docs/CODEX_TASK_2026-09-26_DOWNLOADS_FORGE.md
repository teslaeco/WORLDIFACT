...
1. Reproduce the export flow in source. The current optional artifact path performs GET -> POST `/exports/prepare` -> immediate GET and the server rate-limits artifact GETs through one shared bucket. Remove that self-throttling pattern. For PBR/texture ZIP, FBX and BLEND, prepare the existing saved job at most once per client/job and then do one artifact GET. If preparation is unavailable, allow one read of an already-existing export. Keep GLB read-only. Never call `/v1/jobs`, Astra/OpenAI, checkout, or any paid generation path.
2. Isolate artifact rate limiting by signed job ID and requested format so consecutive PBR/FBX/BLEND downloads do not block each other. Preserve receipt/account ownership checks and same-origin protection.
...
Acceptance: no new AI request; no paid generation; no secret exposure; exact saved GLB remains unchanged; PBR/FBX/BLEND buttons cannot self-rate-limit via an immediate repeated artifact GET; same-origin Forge worker is build- and release-verified.
