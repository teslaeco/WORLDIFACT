...
Seven additional tests cover missing secrets/no network, cost gates, input redaction, a fixed read-only model endpoint, rejection of bad provider results, secure subprocess transfer and truthful result status. The initial PR CI failed TypeScript because the new script was untyped. The completion converts it to TypeScript, narrows unknown errors and metadata, and explicitly rejects whitespace in API keys (including a trailing newline). The local completion passed all 48 tests, lint, TypeScript, real local DEMO HTTP, build, Cloudflare packaging and diff checks. GitHub Actions records CI on the published commit and the actual deployment result.
