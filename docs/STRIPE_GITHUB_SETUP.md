...
Saving secrets is preparation, not proof of payment settlement. After successful setup the owner explicitly requested Stripe activation. The activation release sets `ENABLE_BILLING=true`, prepares a dedicated subscription-cancellation portal and checks public readiness plus request guards without a charge. This later instruction supersedes the former activation hold. Provider-sandbox end-to-end settlement, real paid settlement and device-specific Google Pay visibility remain unverified; none is inferred from setup or activation. PayPal needs its own merchant credentials and remains disabled.
