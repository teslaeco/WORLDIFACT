# WORLDIFACT — Oracle export recovery v4 merged/deployed, 26 September 2026

**Status: WEBSITE LIVE · MAIN CI GREEN · ORACLE v4 APPLY/E2E STILL REQUIRES OWNER CLOUD SHELL EXECUTION.**

- PR #121 merged to `main` as `adeb72ec5149f001eb30bc090709f4f53fba22a0`.
- Main CI run `36252333702`: **SUCCESS**.
- Production workflow `36252333633`: **SUCCESS**.
- Live URL: `https://worldifact.xodobrox.workers.dev`.
- Cloudflare version: `f0e79c6f-8edc-4e8f-bcba-3de8ca06294c`.
...
- The v4 Oracle repair is now shipped in the repository, but it is **not yet applied to the VM** until the owner runs `tools/export_prepare/finish_and_verify.py --approve-one-live-test` from OCI Cloud Shell.
- DONE for this P0 requires the Cloud Shell run to end with `WORLDIFACT_EXPORT_E2E_PASS` and `WORLDIFACT_ORACLE_EXPORT_FIX_AND_E2E_COMPLETE`, proving actual GLB + PBR/texture ZIP + FBX + BLEND downloads from one persisted test job.
