...
**Status: IMPLEMENTED ON REVIEW BRANCH · CI / MERGE / PRODUCTION / ORACLE CAPABILITY VERIFICATION PENDING.** Branch: `fix/p0-downloads-forge-same-origin-20260926`.
...
- The client now performs one idempotent, no-AI export-preparation request before the first PBR/FBX/BLEND download, caches that preparation per saved job, and performs exactly one artifact GET. Already-existing exports still get one read if preparation is temporarily unavailable.
- Artifact read limits are now isolated by signed job + format, so PBR, FBX and BLEND do not throttle one another just because the user downloads them consecutively.
...
PR #69 merged as `f204b3b82402763859c8f08d5f918eae75cbcfca` after all five checks and 374/374 CI tests passed. Deployment `35714230806` stopped before publication: Stripe rejected partial optional create objects with HTTP 400. PR #70 corrected the pinned create schema and merged as `4fd025f8e70329ab62b9a4c6b3752e727402cb58` after all five checks and 376/376 CI tests passed. Its deployment `35715318007` successfully created the owned portal but stopped at strict returned-policy validation. PR #71 merged as `900223a616dbbad5f0cc3b61dde01c5f66864b7e` after 379/379 CI tests. Deployment `35716249583` identified the sole mismatch as `is_default`: Stripe marked the newly created owned configuration as default. All other policy checks passed; no completed activation is inferred.
