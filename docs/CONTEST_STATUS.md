...
PR #69 merged as `f204b3b82402763859c8f08d5f918eae75cbcfca` after all five checks and 374/374 CI tests passed. Deployment `35714230806` stopped before publication: Stripe rejected partial optional create objects with HTTP 400. PR #70 corrected the pinned create schema and merged as `4fd025f8e70329ab62b9a4c6b3752e727402cb58` after all five checks and 376/376 CI tests passed. Its deployment `35715318007` successfully created the owned portal but stopped at strict returned-policy validation. The exact mismatched field was not yet exposed; no completed activation is inferred.

The next follow-up reports only fixed policy field names on validation failure. It can normalize the two optional disabled features on the uniquely owned configuration only when every identity, ownership, origin and cancellation guard already passes; arbitrary/default/foreign configurations and all other mismatches remain blocked. New configurations explicitly provide the pinned schema's required empty arrays instead of assuming disabled defaults. Existing owned resources are reused, not duplicated. Local checks: 12/12 focused tests pass; aggregate verify passes lint, TypeScript and 378 tests with the existing Chromium-required test blocked by the missing binary (379 total). Separate HTTP smoke, build, Worker dry-run and diff checks pass. Exact release results will be recorded in the follow-up PR.
