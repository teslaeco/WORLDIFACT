...
PR #69 merged as `f204b3b82402763859c8f08d5f918eae75cbcfca` after all five checks and 374/374 CI tests passed. Deployment `35714230806` stopped before publication: Stripe rejected the portal creation request with HTTP 400. Production therefore still runs version `578619f0-2294-4096-8f0c-91264ef60473` with checkout disabled. The follow-up corrects partial optional objects against the official Stripe 2024-06-20 create schema, validates their disabled defaults in the response, and adds bounded allowlisted provider-error diagnostics without revealing messages or credentials. Local follow-up verification: 375 tests pass, with the one existing Chromium-required test blocked by the missing binary (376 total); lint, TypeScript, HTTP smoke, build, Worker dry-run and diff checks pass. Actual activation remains pending the corrected release. Real paid settlement and a provider sandbox end-to-end purchase remain unverified; automated settlement/refund/duplicate tests use isolated fixtures.
