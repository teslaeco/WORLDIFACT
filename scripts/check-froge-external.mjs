...
export async function inspectWorldifactServices(fetcher = fetch) {
  ...
  for (const path of ['/api/health', '/api/platform/oracle-worlds', '/api/studio/status']) {
    try {
      ...
      for (const key of ['connectorVersion', 'characterStandard', 'posthocExportRevision', 'legacyGlbExportRecoveryRevision', 'projectFilesRevision']) if (Number.isSafeInteger(body[key])) record[key] = body[key]
