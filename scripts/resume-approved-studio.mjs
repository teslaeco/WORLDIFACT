...
export function approvedStudioConfig(base, status, now = Date.now()) {
  ...
  config.vars = { ...config.vars, ENABLE_STUDIO_JOBS: 'true', ENABLE_PAID_GENERATION: 'false', ENABLE_ORACLE_JOBS: 'false', PUBLIC_PILOT: 'true',
    GENERATION_REQUEST_LIMIT: String(APPROVED_CEILING), GENERATION_EXPIRES_AT: new Date(now + 180 * 60_000).toISOString() }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (process.argv[2] === 'verify') {
    if (status.ready !== true || status.photoReady !== true || status.allowance?.limit !== APPROVED_CEILING) throw new Error('Studio is not ready under the original six-attempt ceiling.')
    ...
    console.log(JSON.stringify({ eligible: result.eligible, reason: result.reason || 'RESUME_UNUSED_APPROVED_SLOTS', used: result.used, remaining: result.remaining, cumulativeCeiling: APPROVED_CEILING, paidGenerationRequested: false }))
  }
}
