...
async function boundedJson(response: Response, limit = 16_384): Promise<Record<string, unknown>> {
  ...
  for (;;) {
    ...
    if (size > limit) {
      await reader.cancel();
      throw new Error('UPSTREAM_OVERSIZED');
    }
...
async function oracleFetch(
  ...
  if (!origin || !env.ORACLE_API_TOKEN) throw new Error('NOT_CONFIGURED');
...
async function checkedHealth(env: OracleJobEnv, fetcher: typeof fetch) {
  ...
  if (
    ...
    typeof body.connectorVersion !== 'number' ||
    !Number.isSafeInteger(body.connectorVersion) ||
    body.connectorVersion < 33
  ) throw new Error('ORACLE_NOT_READY');
  return { connectorVersion: body.connectorVersion };
}

function sanitizedJob(body: Record<string, unknown>, expectedId: string) {
  if (body.id !== expectedId || typeof body.state !== 'string' || !JOB_STATES.has(body.state)) {
    throw new Error('UPSTREAM_INVALID_JOB');
  }
...
async function reserveSharedBudget(env: OracleJobEnv) {
  ...
  const response = await budget.fetch(new Request('https://budget.internal/reserve', {
    method: 'POST',
  ...
  if (!response.ok || (await response.json() as { allowed?: boolean }).allowed !== true) {
    throw new Error('BUDGET_DISABLED');
  }
}

async function validatedGlb(response: Response) {
  if (!response.ok) {
    await response.body?.cancel();
    throw new Error(`UPSTREAM_${response.status}`);
  }
  ...
  if (type !== 'model/gltf-binary' && type !== 'application/octet-stream') {
    await response.body?.cancel();
    throw new Error('UPSTREAM_MODEL_TYPE');
  }
  const declared = Number(response.headers.get('content-length') || '0');
  if (declared && (declared < 20 || declared > MAX_GLB_BYTES)) {
    await response.body?.cancel();
    throw new Error('UPSTREAM_MODEL_SIZE');
  }
  const reader = response.body?.getReader();
  if (!reader) throw new Error('UPSTREAM_EMPTY');
  ...
  if (size < 20 || (declared && declared !== size)) throw new Error('UPSTREAM_MODEL_SIZE');
  ...
  if (
    ...
    view.getUint32(4, true) !== 2 || view.getUint32(8, true) !== size
  ) throw new Error('UPSTREAM_MODEL_INVALID');
...
export async function oracleJobApi(request: Request, env: OracleJobEnv, fetcher: typeof fetch = fetch) {
  ...
  try {
    ...
    if (!(await env.GENERATION_LIMITER.limit({ key })).success) {
      return reply({ error: 'Please wait before another Oracle request.' }, 429);
    }
  } catch {
    return reply({ error: 'Oracle job limiter unavailable.' }, 503);
  }

  const modelMatch = /^\/api\/oracle\/jobs\/([a-f0-9-]{36})\/model$/.exec(url.pathname);
  if (modelMatch && request.method === 'GET') {
    if (!UUID.test(modelMatch[1])) return reply({ error: 'Invalid job id.' }, 400);
    try {
      const upstream = await oracleFetch(env, `/v1/jobs/${modelMatch[1]}/model`, { method: 'GET' }, fetcher, 'model/gltf-binary');
      const { bytes, sha256 } = await validatedGlb(upstream);
      return new Response(bytes, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="WORLDIFACT-oracle-${modelMatch[1]}.glb"`,
          'X-WORLDIFACT-SHA256': sha256,
          'X-WORLDIFACT-Provenance': 'GENERATED-UNREVIEWED',
        },
      });
    } catch {
      return reply({ error: 'Oracle GLB artifact is unavailable or invalid.' }, 502);
    }
  }
  ...
  if (match && request.method === 'GET') {
    ...
    try {
      const body = await boundedJson(await oracleFetch(env, `/v1/jobs/${match[1]}`, { method: 'GET' }, fetcher));
      return reply({ job: sanitizedJob(body, match[1]) });
    } catch {
      return reply({ error: 'Oracle job status is unavailable.' }, 502);
    }
  }
  ...
  try {
    input = await inputJson(request);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'INVALID';
    return reply(
      { error: message === 'TOO_LARGE' ? 'Request too large.' : message === 'CONTENT_TYPE' ? 'Use application/json.' : 'Invalid request.' },
      message === 'TOO_LARGE' ? 413 : message === 'CONTENT_TYPE' ? 415 : 400,
    );
  }
  if (
    Object.keys(input).some(k => !['worldId', 'id', 'prompt'].includes(k)) ||
    typeof input.worldId !== 'string' ||
    !ORACLE_WORLD_IDS.includes(input.worldId as (typeof ORACLE_WORLD_IDS)[number]) ||
  ...
  try {
    ...
    if (upstream.status === 409) {
      ...
      return reply({ error: 'Oracle is busy or rejected this job.' }, 409);
    }
    if (upstream.status === 429) {
      await upstream.body?.cancel();
      return reply({ error: 'Oracle rate limit reached.' }, 429);
    }
