...
export interface OracleJobEnv extends PlatformEnv {
...
async function boundedJson(response: Response, limit = 16_384): Promise<Record<string, unknown>> {
  ...
  for (;;) {
    ...
    if (size > limit) { await reader.cancel(); throw new Error('UPSTREAM_OVERSIZED'); }
...
async function inputJson(request: Request): Promise<Record<string, unknown>> {
  ...
  for (;;) {
    ...
    if (done) break;
    size += value.byteLength;
    if (size > 12_000) { await reader.cancel(); throw new Error('TOO_LARGE'); }
    text += decoder.decode(value, { stream: true });
  }
  const value: unknown = JSON.parse(text + decoder.decode());
...
async function checkedHealth(env: OracleJobEnv, fetcher: typeof fetch) {
  ...
  if (body.ready !== true || body.provider !== 'openai' || body.model !== 'gpt-6-astra' ||
    typeof body.connectorVersion !== 'number' || !Number.isSafeInteger(body.connectorVersion) || body.connectorVersion < 33) {
...
export async function oracleJobApi(request: Request, env: OracleJobEnv, fetcher: typeof fetch = fetch) {
  ...
  if (url.pathname === '/api/oracle/jobs/status' && request.method === 'GET') {
    return reply({
      ...
      note: enabled
        ? 'Owner-only Oracle job submission is enabled. OpenAI/Blender work may incur provider cost.'
        : 'Oracle write calls are disabled. Read-only health remains available for all five worlds.',
    });
  }
  ...
  try { input = await inputJson(request); }
    ...
    return reply({ error: message === 'TOO_LARGE' ? 'Request too large.' : message === 'CONTENT_TYPE' ? 'Use application/json.' : 'Invalid request.' },
      message === 'TOO_LARGE' ? 413 : message === 'CONTENT_TYPE' ? 415 : 400);
  }
  if (Object.keys(input).some(k => !['worldId', 'id', 'prompt'].includes(k)) ||
    typeof input.worldId !== 'string' || !ORACLE_WORLD_IDS.includes(input.worldId as (typeof ORACLE_WORLD_IDS)[number]) ||
    typeof input.id !== 'string' || !UUID.test(input.id) ||
    typeof input.prompt !== 'string' || input.prompt.trim().length < 3 || input.prompt.length > 2000) {
  ...
  try {
    ...
    return reply({
      ...
      evidence: 'Oracle accepted an owner-authorized prompt job. The browser never received the Oracle endpoint or bearer token.',
    }, upstream.status === 202 ? 202 : 200);
  } catch (error) {
    return reply({ error: error instanceof Error && error.message === 'ORACLE_NOT_READY'
      ? 'Oracle is not ready for the reviewed Astra job contract.'
      : 'Oracle job submission failed safely.' }, 502);
  }
}
