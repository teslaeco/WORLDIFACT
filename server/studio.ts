...
export async function studioApi(request: Request, env: StudioEnv, fetcher: typeof fetch = fetch): Promise<Response> {
  ...
  try {
      ...
      try {
        ...
        if ([400, 409, 422, 429].includes(response.status)) {
          ...
          const message = response.status === 409
            ? 'Oracle is finishing another model. Your reserved allowance or credits were restored. Try this same draft again when the worker is ready.'
            : response.status === 429
    ...
    const reconcileMatch = new RegExp(`^/api/studio/jobs/(${UUID})/reconcile-missingimport { oracleOrigin, ownerAuthorized, type PlatformEnv } from './platform.ts'
import { getVerifiedAccount, type AccountEnv } from './accounts.ts'
import { reserveUserGeneration, settleUserGeneration, userJobAccess, EntitlementError, type EntitlementEnv } from './entitlements.ts'
import { budgetSettings, APPROVED_FAST_TEST, type BudgetEnv, type BudgetNamespace } from './budget.ts'
import { inputDigest, oracleStudioPayload, validateStudioInput, supportsFastDraft, FAST_DRAFT_PROFILE, STUDIO_BODY_LIMIT, STUDIO_MODEL_LIMIT, STUDIO_RECONCILIATION_DETAIL, JOB_DETAILS, type StudioInput, type StudioJob } from '../src/lib/studioProtocol.ts'
...
export async function studioApi(request: Request, env: StudioEnv, fetcher: typeof fetch = fetch): Promise<Response> {
  ...
  try {
    ...
    if (reconcileMatch && request.method === 'POST') {
      const user = await accountIdentity(request, env, fetcher)
      if (!user) throw new StudioError('A signed-in account is required to reconcile this job.', 401)
      const auth = await verifyReceipt(env, request.headers.get('X-WORLDIFACT-Job') || '', reconcileMatch[1], true, user.id)
      await accountAccess(env, user.id, auth.id)
      if (Date.now() - auth.issued < 180_000) throw new StudioError('Wait for the original submission recovery window before reconciling this job.', 409)
      await limit(request, env, `reconcile:${auth.id}`)
      const response = await oracle(env, `/v1/jobs/${auth.id}`, fetcher)
      if (response.status === 404) {
        await response.body?.cancel()
        const job = await accountJob(env, user.id, auth.id, 'failed')
        return json({ job, reconciled: true, generationRequested: false, paidGenerationRequested: false })
      }
      if (!response.ok) {
        await response.body?.cancel()
        throw new StudioError('The worker could not confirm this job yet. Keep the same receipt and retry reconciliation.', response.status === 429 ? 429 : 502)
      }
      const value = await limitedJson(response, 16_384)
      if (value.id !== auth.id || !Object.hasOwn(JOB_DETAILS, String(value.state))) throw new StudioError('The worker returned an invalid job status.', 502)
      return json({ job: await accountJob(env, user.id, auth.id, value.state as StudioJob['state']), reconciled: false, generationRequested: false, paidGenerationRequested: false })
    }

    const prepareMatch = new RegExp(`^/api/studio/jobs/(${UUID})/exports/prepareimport { oracleOrigin, ownerAuthorized, type PlatformEnv } from './platform.ts'
import { getVerifiedAccount, type AccountEnv } from './accounts.ts'
import { reserveUserGeneration, settleUserGeneration, userJobAccess, EntitlementError, type EntitlementEnv } from './entitlements.ts'
import { budgetSettings, APPROVED_FAST_TEST, type BudgetEnv, type BudgetNamespace } from './budget.ts'
import { inputDigest, oracleStudioPayload, validateStudioInput, supportsFastDraft, FAST_DRAFT_PROFILE, STUDIO_BODY_LIMIT, STUDIO_MODEL_LIMIT, STUDIO_RECONCILIATION_DETAIL, JOB_DETAILS, type StudioInput, type StudioJob } from '../src/lib/studioProtocol.ts'
