...
export interface StudioEnv extends PlatformEnv, BudgetEnv { PUBLIC_PILOT?: string; GENERATION_BUDGET?: BudgetNamespace }
...
class StudioError extends Error { constructor(message: string, readonly status = 400) { super(message) } }
...
async function limitedJson(response: Request | Response, limit: number) {
  if (!response.headers.get('content-type')?.toLowerCase().startsWith('application/json')) throw new StudioError('Expected an application/json response.', 415)
  const declared = Number(response.headers.get('content-length') || 0)
  if (declared > limit) throw new StudioError('Request or response is too large.', 413)
  ...
  try {
    ...
    throw new StudioError('Invalid JSON data.')
  }
}
async function limit(request: Request, env: StudioEnv, bucket: string) {
  ...
  } catch (error) { if (error instanceof StudioError) throw error; throw new StudioError('Request limiter is unavailable.', 503) }
}
...
async function health(env: StudioEnv, fetcher: typeof fetch) {
  ...
  if (!response.ok) { await response.body?.cancel(); throw new StudioError('The existing Oracle worker did not confirm readiness.', 503) }
...
async function preflight(request: Request, env: StudioEnv, fetcher: typeof fetch, input: StudioInput) {
  if (env.ENABLE_ORACLE_JOBS !== 'true' || !budgetSettings(env)) throw new StudioError('Model generation is currently disabled or its allowance has expired.', 503)
  ...
  if (input.photos.length && !current.photoReady) throw new StudioError('This connected worker has not confirmed photo input. Nothing was submitted.', 409)
  if (oracleStudioPayload('', input).prompt.length > current.promptMaxLength) throw new StudioError(`Shorten the description: the connected worker accepts ${current.promptMaxLength} characters including export instructions.`, 400)
  return current
}
...
async function modelOrExport(env: StudioEnv, id: string, format: string, fetcher: typeof fetch) {
  ...
  const reader = response.body.getReader()
  // Verify the GLB header before returning success, without buffering 48 MB twice.
  const initial: Uint8Array[] = []; let initialSize = 0
  if (model) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async pull(controller) {
      try {
        const value = initial.shift()
        const next = value ? { done: false, value } : await reader.read()
      ...
      } catch { await reader.cancel().catch(() => {}); controller.error(new Error('The artifact stream was interrupted. Retry the same artifact, not generation.')) }
    },
...
export async function studioApi(request: Request, env: StudioEnv, fetcher: typeof fetch = fetch): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === '/api/studio/status' && request.method === 'GET') {
      ...
      try { pool = await allowance(env) } catch { /* Keep reasons distinct from exhausted. */ }
      try { state = await health(env, fetcher) } catch { /* Configuration is not a successful generation. */ }
      const enabled = env.ENABLE_ORACLE_JOBS === 'true' && !!budgetSettings(env)
    ...
    if (url.pathname === '/api/studio/prepare' && request.method === 'POST') {
      ...
      const input = validateStudioInput(await limitedJson(request, STUDIO_BODY_LIMIT))
      await preflight(request, env, fetcher, input)
      const pool = await allowance(env)
      if (!pool.remaining) throw new StudioError('The cumulative generation allowance is exhausted. No job was started.', 429)
      // Creating a receipt is free and does not reserve a paid attempt. The
      // browser stores this identifier BEFORE submitting the paid job.
    ...
    if (url.pathname === '/api/studio/jobs' && request.method === 'POST') {
      ...
      const input = validateStudioInput(await limitedJson(request, STUDIO_BODY_LIMIT))
      if (await inputDigest(input) !== auth.hash) throw new StudioError('The inputs changed after this receipt was prepared. Nothing was submitted.', 409)
      await preflight(request, env, fetcher, input)
      ...
      try {
        ...
        // Never resend the paid request. Even a timeout consumed its original
        // reservation; the signed receipt recovers this exact job by GET.
    ...
    if (match && request.method === 'GET') {
      ...
      if (response.status === 404) {
        ...
        const state = Date.now() - auth.issued < 180_000 ? 'pending' : 'failed'
        return json({ job: { id: auth.id, state, detail: JOB_DETAILS[state] } })
      }
      if (!response.ok) { await response.body?.cancel(); throw new StudioError('Status is temporarily unavailable. Keep polling this same job.', response.status === 429 ? 429 : 502) }
      const value = await limitedJson(response, 16_384)
    ...
    // Validation messages are authored locally. Never forward upstream bodies,
    // stack traces, URLs, credentials, prompts or photo payloads.
    return json({ error: 'The Studio request could not be confirmed. Preserve your inputs and receipt; do not resubmit a paid job automatically.' }, 503)
  }
}
