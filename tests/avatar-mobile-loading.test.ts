...
test('a progressing mobile download survives 60 seconds and reports actual decoded progress', async t => {
  t.mock.timers.enable({ apis: ['setTimeout'] })
  const original = globalThis.fetch; clearAvatarAssets()
  const states: AvatarProgress[] = []; const unsubscribe = subscribeAvatarProgress('queen', state => states.push(state))
  let controller!: ReadableStreamDefaultController<Uint8Array>; let signal!: AbortSignal; let calls = 0
  globalThis.fetch = (async (_url, init) => {
    calls++; signal = init!.signal!
    const body = new ReadableStream<Uint8Array>({ start(value) { controller = value } })
    // Compressed wire length differs from the automatically decoded GLB length.
    return new Response(body, { headers: { 'Content-Encoding': 'gzip', 'Content-Length': '12', 'X-WORLDIFACT-GLB-Length': '20' } })
  }) as typeof fetch
  try {
    const pending = loadAvatarBytes('queen'); await flush()
...
test('one transient failure retries automatically and still shares the original in-flight promise', async t => {
  t.mock.timers.enable({ apis: ['setTimeout'] })
  ...
  try {
    const pending = loadAvatarBytes('queen'); assert.equal(pending, loadAvatarBytes('queen')); await flush()
...
test('logout during retry backoff cancels the retry and discards the old session request', async t => {
  ...
  try {
    const pending = loadAvatarBytes('queen'); const rejected = assert.rejects(pending)
...
test('a genuinely stalled stream is aborted and retries, unlike a healthy slow stream', async t => {
  ...
  try {
    const pending = loadAvatarBytes('queen'); await flush(); t.mock.timers.tick(30_001); await flush()
...
test('progress subscription cleanup and terminal errors do not manufacture a successful model', async () => {
  ...
  const unsubscribe = subscribeAvatarProgress('queen', () => notifications++)
  unsubscribe()
  globalThis.fetch = (async () => { calls++; return new Response('<html>login</html>', { status: 403 }) }) as typeof fetch
  try { await assert.rejects(loadAvatarBytes('queen')); assert.equal(calls, 1); assert.equal(notifications, 1) }
