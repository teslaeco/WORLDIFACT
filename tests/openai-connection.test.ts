...
const validConfig = (): { name: string; vars: Record<string, string> } => ({ name: "worldifact", vars: {
  OPENAI_MODEL: "gpt-6-astra", ENABLE_PAID_GENERATION: "false", PUBLIC_PILOT: "false", ENABLE_ORACLE_JOBS: "false",
...
test("automation refuses paid or altered production configuration", () => {
  ...
  for (const [field, value] of [["ENABLE_PAID_GENERATION", "true"], ["PUBLIC_PILOT", "true"], ["ENABLE_ORACLE_JOBS", "true"],
...
test("secret validation never includes rejected values or enables billing", () => {
  ...
  assert.deepEqual(readSecretInputs({ OPENAI_API_KEY: key, GENERATION_ACCESS_TOKEN: access }), { OPENAI_API_KEY: key, GENERATION_ACCESS_TOKEN: access });
  for (const invalid of [`Bearer ${key}`, `${key}\n`, `"${key}"`]) {
    assert.throws(() => readSecretInputs({ OPENAI_API_KEY: invalid }), (error) => {
      assert.ok(error instanceof Error); assert.ok(!error.message.includes(key)); return true;
    });
  }
...
test("model access uses only a fixed read endpoint and refuses redirects", async () => {
  ...
  await verifyModelAccess(key, async (url, options) => {
    calls++; assert.equal(url, "https://api.openai.com/v1/models/gpt-6-astra");
    assert.equal(options.method, "GET"); assert.equal(options.redirect, "error");
    assert.equal(new Headers(options.headers).get("Authorization"), `Bearer ${key}`);
    assert.equal(options.body, undefined); assert.ok(options.signal instanceof AbortSignal); return ok();
  });
  assert.equal(calls, 1);
});

test("failed model access never uploads secrets or exposes provider error text", async () => {
  let uploads = 0; const upload = () => { uploads++; };
  for (const fetcher of [async () => { throw new Error(key); }, async () => new Response(key, { status: 401 }), async () => new Response(key, { status: 200 }), async () => Response.json({ id: "other", object: "model", secret: key })]) {
...
test("Worker synchronization uses stdin, not shell arguments, files or child secret environment", () => {
  const payload = { OPENAI_API_KEY: key, GENERATION_ACCESS_TOKEN: access }; let calls = 0;
  uploadWorkerSecrets(payload, (command, args, options) => {
    calls++; assert.equal(command, process.execPath); assert.deepEqual(args.slice(1), ["secret", "bulk", "--name", "worldifact"]);
    assert.ok(!args.join(" ").includes(key)); assert.equal(options.shell, false); assert.deepEqual(JSON.parse(options.input), payload);
    assert.equal(options.env.OPENAI_API_KEY, undefined); assert.equal(options.env.GENERATION_ACCESS_TOKEN, undefined);
    assert.equal(options.env.CLOUDFLARE_API_TOKEN, "test-cloudflare-token"); return { status: 0 };
...
test("successful simulated provisioning is CONFIGURED, never LIVE evidence", async () => {
  ...
  const result = await connectOpenAI({ OPENAI_API_KEY: key, GENERATION_ACCESS_TOKEN: access }, { fetcher: async () => ok(), upload: (payload) => { uploaded = payload; } });
  assert.deepEqual(uploaded, { OPENAI_API_KEY: key, GENERATION_ACCESS_TOKEN: access });
  assert.equal(result.status, "CONFIGURED"); assert.match(result.reason, /not LIVE/); assert.ok(!JSON.stringify(result).includes(key));
});
