...
test('Oracle maintenance prints a heartbeat while waiting and never cancels or mutates active jobs', async () => {
  const source = await readFile(new URL('../tools/export_prepare/oracle_direct_export_fix.py', import.meta.url), 'utf8')
  ...
  assert.match(source, /def queue_status\(\):/)
  assert.match(source, /"phase":"IDLE" if not jobs else "ACTIVE"/)
  assert.match(source, /elif MODE=="queue-status"/)
  assert.doesNotMatch(source, /UPDATE jobs SET state/)
  assert.match(finisher, /call_remote\(key,"queue-status"/)
  assert.match(finisher, /Oracle queue ACTIVE:/)
  assert.match(finisher, /waiting 15 s/)
  assert.match(finisher, /time\.monotonic\(\)\+1500/)
