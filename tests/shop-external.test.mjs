...
test('native Shop presents a customer creation flow with cart and no visible engineering controls', async () => {
  ...
  assert.doesNotMatch(html, /X width \(mm\)/)
  assert.doesNotMatch(html, /Y height \(mm\)/)
  assert.doesNotMatch(html, /Z depth \(mm\)/)
  const manufacturingSource = await readFile(new URL('../src/components/ShopManufacturingOptions.tsx', import.meta.url), 'utf8')
  assert.match(manufacturingSource, /dimensionsEnabled &&/)
  assert.match(manufacturingSource, /X width \(mm\)/)
  assert.match(manufacturingSource, /Y height \(mm\)/)
  assert.match(manufacturingSource, /Z depth \(mm\)/)
