...
const CODE = 'one-time-google-code-fixture'
...
function fixture() {
  ...
  const fetcher = (async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
    ...
    assert.ok(headers.get('apikey')?.startsWith('sb_publishable_'))
    ...
    if (url.endsWith('/token?grant_type=pkce')) {
      ...
      used = true
      return Response.json({ access_token: ACCESS, refresh_token: RENEWAL, expires_in: 3600, provider_token: 'google-api-token-must-be-discarded', user })
    }
...
test('Expired, malformed and future OAuth state cookies fail closed before provider exchange', async () => {
  ...
  ]) assert.equal(destination(await f.callback(started.flow.state, cookie)).searchParams.get('oauth'), 'error')
  assert.equal(f.calls.length, 0)
})
test('Provider cancellation and duplicate or malformed codes are sanitized and clear only the matching OAuth flow', async () => {
  ...
  const f = fixture(), started = await f.start()
  assert.equal(destination(await f.callback(started.flow.state, started.cookie, 'bad')).searchParams.get('oauth'), 'error')
  assert.equal(f.calls.length, 0)
})
test('Failed code exchange, rejected tokens and mismatched provider user cannot install a session', async () => {
  for (const failure of ['network', 'exchange', 'user', 'mismatch']) {
...
test('OAuth callback is rate limited and a consumed Supabase code cannot create another session', async () => {
  ...
  assert.equal(destination(await blocked.callback(started.flow.state, started.cookie)).searchParams.get('oauth'), 'error')
