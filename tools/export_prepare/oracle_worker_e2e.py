"""One explicit live Oracle worker generation + GLB/PBR/FBX/BLEND verification.

Run only after direct export v2 is installed. This submits exactly one new model
job and never retries generation. Export preparation reuses the same saved job
and does not request AI.
"""
import hashlib,json,os,struct,sys,time,urllib.error,urllib.request,uuid
from pathlib import Path

ROOT=Path.home()/"froge-connector"
OUT=Path.home()/".local/state/worldifact-direct-export-v3/e2e"
RESULT=OUT/"result.json"
BASE="http://127.0.0.1:8765"

def atomic_result(value):
...
def token():
    ...
    value=json.loads(path.read_text());v=value.get("token")
...
def save(name,raw):
    ...
    path=folder/name;path.write_bytes(raw);os.chmod(path,0o600)
    return str(path)
def verify_glb(raw):
...
result={"phase":"E2E_RUNNING","jobId":JOB_ID,"paidGenerationRequested":True,"generationRequested":True,"automaticGenerationRetries":0}
...
try:
    ...
    status,submitted=json_request("/v1/jobs","POST",{"id":JOB_ID,"prompt":prompt},45)
    if status not in (200,202) or submitted.get("id")!=JOB_ID:raise RuntimeError("Oracle rejected the one explicit test generation.")
    deadline=time.monotonic()+900
    state=submitted
    while state.get("state") not in ("succeeded","failed","cancelled") and time.monotonic()<deadline:
        time.sleep(12)
        s,state=json_request("/v1/jobs/"+JOB_ID,timeout=35)
        if s!=200:raise RuntimeError("Oracle job status read failed.")
    if state.get("state")!="succeeded":raise RuntimeError("Oracle test generation ended as "+str(state.get("state")))
    artifacts=[]
    ...
    verify_glb(model);artifacts.append({"format":"glb","bytes":len(model),"sha256":sha(model),"path":save("model.glb",model)})
    missing=[]
    downloaded={}
    ...
    for name,filename in (("pbr","textures-pbr.zip"),("fbx","model.fbx"),("blend","model.blend")):
        raw=downloaded[name];artifacts.append({"format":name,"bytes":len(raw),"sha256":sha(raw),"path":save(filename,raw)})
    result={"phase":"E2E_PASS","jobId":JOB_ID,"state":"succeeded","paidGenerationRequested":True,"generationRequested":True,
            "automaticGenerationRetries":0,"artifacts":artifacts}
    atomic_result(result);print(json.dumps(result,indent=2))
except BaseException as exc:
    result={"phase":"E2E_FAILED","jobId":JOB_ID,"paidGenerationRequested":True,"generationRequested":True,
            "automaticGenerationRetries":0,"error":str(exc)[:700]}
    atomic_result(result);print(json.dumps(result,indent=2));sys.exit(1)
